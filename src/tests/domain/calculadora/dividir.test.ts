...
describe('Debe dividir', () => {
    ...
    test('debe dividir 2 valores positivos', () => {
        ...
        expect(dividir({numero1: 9,numero2: 2}).resultado).toBe(4.5);
    });

    test('debe dividir 1 numero negativo y otro positivo', () => {  
        redondearADosDecimalesMock.mockReturnValueOnce(-3);
        redondearADosDecimalesMock.mockReturnValueOnce(3);      
        expect( dividir({ numero1: -3, numero2: 3}).resultado).toBe(-1)
    });
    ...
    test('debe dividir valores decimales', () => {
        ...
        expect( dividir({ numero1: 5.605678, numero2: 3.744567}).resultado).toBe(1.5);       
     });

     test('debe mostrar un error cuando se divide entre 0', () => {
        expect(dividir({ numero1: 3, numero2: 0}).error).toBe('La division por 0 no esta difinida');
     });    
